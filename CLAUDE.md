...
ludus deploy [fleet|stack|anywhere|session|destroy]  --target, --region, --instance-type, --fleet-name, --stack-name, --ip
...
**Tools** (15 total): `ludus_init`, `ludus_status`, `ludus_engine_setup`, `ludus_engine_build`, `ludus_engine_push`, `ludus_game_build`, `ludus_game_client`, `ludus_container_build`, `ludus_container_push`, `ludus_deploy_fleet`, `ludus_deploy_stack`, `ludus_deploy_anywhere`, `ludus_deploy_session`, `ludus_deploy_destroy`, `ludus_connect_info`.
...
- **`wrapper`** — Shared package for the Amazon GameLift Game Server Wrapper binary. `EnsureBinary()` clones the wrapper repo, builds it, and caches at `~/.cache/ludus/game-server-wrapper/`. Used by both `container` (for Docker builds) and `anywhere` (for local server launch).
...
- **Pluggable targets**: Deployment is abstracted behind `deploy.Target` interface. `cmd/globals.ResolveTarget()` is the factory that creates the appropriate target based on config (`deploy.target` in `ludus.yaml`) or CLI flag (`--target`). Implementations: `gamelift` (container fleet), `stack` (CloudFormation), `binary` (file export), `anywhere` (local dev). The pipeline checks `target.Capabilities()` to skip container/push stages for targets that don't need them (binary and anywhere skip containers). GameLift-specific commands (`fleet`, `session`) still use the direct `gamelift.Deployer` when needed; generic commands (`destroy`, pipeline deploy) use the interface.
...
Config template: `ludus.example.yaml`. User config: `ludus.yaml` (gitignored). Key settings: engine source path, max compile jobs (0 = auto-detect from RAM), engine build backend (`native` or `docker`), engine Docker image (pre-built URI or local name), Docker base image (`ubuntu:22.04` default, supports apt-get and dnf bases), project name (`Lyra` default), server map (`L_Expanse`), server port (7777 UDP), deploy target (`gamelift` default, `stack`, `binary`, or `anywhere`), GameLift instance type (`c6i.large`), container group name, AWS region/account, Anywhere config (location name, IP address, AWS profile).
...
- ~~Build caching~~ — `internal/cache/` package with `Cache` struct, per-stage key functions, and `.ludus/cache.json` persistence. Each pipeline stage checks cache before running; skips if input hash matches. `--no-cache` flag to force rebuild. Integrated into engine, game, container CLI commands, pipeline, and MCP tools.
- ~~Configurable Docker base image~~ — `engine.dockerBaseImage` config field (default `ubuntu:22.04`) and `--base-image` CLI flag on `ludus engine build`. Generated Dockerfile auto-detects package manager at build time (`apt-get` for Debian/Ubuntu, `dnf` for Amazon Linux/RHEL/Fedora).
- ~~GameLift Anywhere~~ — `ludus deploy anywhere` creates an Anywhere fleet, registers the local machine, and launches the game server locally via the Game Server Wrapper. Fleet creation takes seconds (vs 15-30 min for container fleets). No container build or ECR push required. `internal/anywhere/` package with `Deployer` and `TargetAdapter`. Shared `internal/wrapper/` package extracted from container builder. MCP tool `ludus_deploy_anywhere` added (15 tools total).
