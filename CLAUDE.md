...
ludus doctor                       # deep diagnostics: toolchain, stale artifacts, state, cache, disk, AWS creds, Docker, Dockerfile security, git
...
- **`game`** — `Builder` for UE5 game packaging via RunUAT BuildCookRun. Cross-platform: `resolveRunUAT()` selects `cmd /c RunUAT.bat` (Windows) or `bash RunUAT.sh` (Linux). Uses relative script paths to avoid spaces-in-path issues with `cmd /c`. Path arguments are quoted (`-project="..."`) for the same reason. Pre-build fixups: `applyNuGetAuditWorkaround()` sets `NuGetAuditLevel=critical` as an env var on the runner (avoids writing `Directory.Build.props` into engine source; version-gated to 5.6/unknown), and `ensureDefaultServerTarget()` configures DefaultEngine.ini (game project config, not engine source; skips gracefully if INI structure doesn't match). `BuildClient()` supports `--platform` flag (Linux, Win64). All target names (`-servertargetname`, binary paths) are config-driven via `BuildOptions`. `EngineVersion` in `BuildOptions` enables version-specific workarounds. `PartialBuildHint()` and `PartialClientBuildHint()` detect cooked content from a previous build (in `Saved/Cooked/<Platform>/`) and suggest `--skip-cook` when the final output is missing (saves 30-60 min on crash recovery).
- **`container`** — `Builder` for Dockerfile generation (Amazon Linux 2023, non-root user), `docker build`, and ECR push (login + tag + push). Project name, server target, and architecture are parameterized in generated Dockerfile and wrapper config. `BuildOptions.Arch` controls wrapper binary architecture, binary paths (`Binaries/Linux/` vs `Binaries/LinuxArm64/`), and `docker build --platform` flag (`linux/amd64` or `linux/arm64`). Defaults to `amd64`.
- **`dflint`** — Dockerfile security linting and image vulnerability scanning. `LintDockerfile(content)` runs 4 built-in rules (no-root-user, unpinned-base-image, no-package-cleanup, sensitive-env) plus optional Hadolint integration (if installed). `LintImage(imageRef)` runs optional Trivy image scan (if installed). `Finding` type with Source/Rule/Line/Level/Message. `LintResult` with `HasErrors()`, `HasWarnings()`, `Summary()` methods. Integrated into `ludus doctor` (game + engine Dockerfile lint, container image scan), `ludus container build` (post-build lint), and the pipeline.
...
- **Cache miss reasons**: All build commands and pipeline stages print why a rebuild is happening: "no previous build recorded" or "inputs changed since last build (<timestamp>)". Partial build detection in game builds checks for cooked content (`Saved/Cooked/<Platform>/`) and suggests `--skip-cook` when the final output is missing — common after OOM/crash during compilation.
...
- **Features** — ~~ARM/Graviton support~~ (PR #36), ~~ARM64 container builds~~, ~~npm package for MCP distribution~~, BuildGraph XML generation, studio infrastructure provisioning
