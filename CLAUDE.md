...
- **`pricing`** — Static instance-type pricing lookup. `EstimateCost(instanceType)` returns hourly USD cost. `FormatEstimate(instanceType)` returns a human-readable string (e.g., `"Estimated cost: $0.085/hr (~$61/month) for c6i.large"`). Covers c5/c6i/c6g/c7g (compute), m5/m6i/m6g (general), r5/r6i (memory) families. Used by deploy commands, pipeline, and MCP tools.
...
- **Cost estimates**: Deploy commands (`fleet`, `stack`, `ec2`) and the pipeline print estimated hourly/monthly cost from a static pricing table (`internal/pricing`) before fleet creation. MCP deploy results include `estimated_cost_per_hour` field. Unknown instance types are silently skipped.
