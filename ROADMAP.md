...
- [x] **Studio infrastructure provisioning** — Now a separate project: **[Fabrica](https://github.com/jpvelasco/fabrica)** — a CLI tool and IaC framework that provisions game studio infrastructure on AWS (Perforce Helix Core, Horde build farms, CI/CD pipelines, deployment targets, cost management). Fabrica picks up where Ludus leaves off: Ludus builds the game and generates BuildGraph XML; Fabrica provisions the infrastructure to execute those builds at scale. See `PRODUCT_SPEC.md` in the Fabrica repo.
