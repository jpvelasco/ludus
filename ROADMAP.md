...
- [ ] **ARM64 container builds for GameLift** — Extend Docker/container path to support ARM64 images for GameLift container fleets. Engine Docker image builder, game container Dockerfile generation, and ECR push need multi-arch support (docker buildx). Enables `ludus container build --arch arm64` → `ludus deploy fleet` on Graviton container instances.
- [ ] **Async MCP build tools** — Split long-running MCP tools (`ludus_game_build`, `ludus_game_client`, `ludus_engine_build`) into start/status pairs (e.g., `ludus_game_build_start` returns immediately with a build ID, `ludus_game_build_status` polls for completion). Allows AI agents to start a build, do other work, and check back periodically instead of blocking for hours.
